...
public class DiagnosticsRuleChecker : IRuleChecker
{
    ...
    {
        var expectedLogMessage = _config.GetParameter("ActivityStartLog", "ExpectedLogMessage", "{ActivityName} started");
        var severity = _config.GetParameter("ActivityStartLog", "Severity", "Warn");
        var allowedLogLevels = _config.GetStringArrayParameter("ActivityStartLog", "AllowedLogLevels", ["INFO", "DEBUG"]);
            ...
            if (secondExecutableItem is not null && IsLogWriter(secondExecutableItem))
            {
                if (!allowedLogLevels.Contains(secondExecutableItem.LogMode, StringComparer.OrdinalIgnoreCase))
                {
                    return new RuleCheckResult
                    {
                        ...
                        Source = $"{activity.RootPath}/{activity.Name}",
                        Status = ParseSeverity(severity),
                        Comments = $"Activity starts with 'Try' block followed by Log but does not match allowed log levels: {string.Join(", ", allowedLogLevels)}."
                    };
                }

                if (MatchesExpectedLogMessage(secondExecutableItem, expectedLogMessage.Replace("{ActivityName}", activity.Name), requireExactMatch, caseSensitive))
                {
                    return new RuleCheckResult
                    {
                        Rule = rule,
                        ...
                        Comments = $"Activity starts with 'Try' block followed by Log: '{secondExecutableItem.LogMessage}'."
                    };
                }
            }
        }
        ...
        return new RuleCheckResult
        {
            ...
            Source = $"{activity.RootPath}/{activity.Name}",
            Status = ParseSeverity(severity),
            Comments = $"Activity should start with 'Try' block followed by Log containing: '{expectedLogMessage.Replace("{ActivityName}", activity.Name)}' with allowed log levels: {string.Join(", ", allowedLogLevels)}."
        };
    }

    private RuleCheckResult HasEndLog(Activity activity, Rule rule)
    {
        var expectedLogMessage = _config.GetParameter("ActivityEndLog", "ExpectedLogMessage", "{ActivityName} completed");
        ...
        foreach (var connection in connections)
        {
            ...
            if (currentExecutableItem is not null && IsLogWriter(currentExecutableItem) &&
                MatchesExpectedLogMessage(currentExecutableItem, expectedLogMessage.Replace("{ActivityName}", activity.Name), requireExactMatch, caseSensitive) &&
            ...
            return new RuleCheckResult
            {
                ...
                Source = $"{activity.RootPath}/{activity.Name}",
                Status = ParseSeverity(severity),
                Comments = $"Activity paths should end with Log containing: '{expectedLogMessage.Replace("{ActivityName}", activity.Name)}' with allowed log levels: {string.Join(", ", allowedLogLevels)}."
            };
        }
        ...
        return new RuleCheckResult
        {
            ...
            Source = $"{activity.RootPath}/{activity.Name}",
            Status = RuleCheckStatus.Pass,
            Comments = $"Activity paths end with Log containing: '{expectedLogMessage.Replace("{ActivityName}", activity.Name)}'."
        };
    }
    ...
    private RuleCheckResult HasExceptionLog(Activity activity, Rule rule)
    {
        ...
        var allowedLogLevels = _config.GetStringArrayParameter("ExceptionLog", "AllowedLogLevels", ["WARN", "ERROR", "EXCEPTION"]);
            ...
            if (errorOutId.HasValue && errorOutId != Guid.Empty)
            {
                ...
                if (!allowedLogLevels.Contains(logWriterItem?.LogMode, StringComparer.OrdinalIgnoreCase))
                {
                    return new RuleCheckResult
                    {
                        ...
                        Source = $"{activity.RootPath}/{activity.Name}",
                        Status = ParseSeverity(severity),
                        Comments = $"Incorrect log mode: {logWriterItem?.LogMode} for exception log message. Allowed levels: {string.Join(", ", allowedLogLevels)}"
                    };
                }
            }
        }
        ...
        return new RuleCheckResult
        {
            ...
            Source = $"{activity.RootPath}/{activity.Name}",
            Status = RuleCheckStatus.Pass,
            Comments = $"Log used immediately after 'On Error' port of 'Try-Catch' with appropriate log level and content"
        };
    }
    ...
    private RuleCheckResult HasErrorPortsUsed(Activity activity, Rule rule)
    {
        ...
        var requireErrorMessagePortMapping = _config.GetParameter("ErrorPortUtilization", "RequireErrorMessagePortMapping", true);
    ...
    private RuleCheckResult HasNonEmptyLogs(Activity activity, Rule rule)
    {
        ...
        {
            if (logItem?.LogMessage is null || string.IsNullOrWhiteSpace(logItem.LogMessage) || logItem.LogMessage.Length < minLogLength)
            {
                return new RuleCheckResult
                {
                    Rule = rule,
    ...
    private RuleCheckResult HasComponentErrorHandlerComment(Activity activity, Rule rule)
    {
        ...
        List<ExecutableItem> executableItems = activity.Items.OfType<ExecutableItem>().ToList();

        foreach (ExecutableItem item in executableItems)
        {
            if (item.OnErrorAction != OnErrorAction.Inherit || item.OnErrorActionAfterRetry != OnErrorAction.Inherit)
            {
                ...
                if (connection is null)
                {
                    return new RuleCheckResult
                    {
                        ...
                        Source = $"{activity.RootPath}/{activity.Name}",
                        Status = ParseSeverity(severity),
                        Comments = $"Component '{(string.IsNullOrEmpty(item.Name) ? item.Type : item.Name)}' has modified error handling property but lacks a comment."
                    };
                }
            }
        }
    ...
    private static bool MatchesExpectedLogMessage(ExecutableItem? item, string expectedMessage, bool requireExactMatch, bool caseSensitive)
    {
