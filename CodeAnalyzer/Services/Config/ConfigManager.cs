...
public static class ConfigManager
{
    ...
    {
        var fullPath = Path.Combine(ConfigFolderPath, $"{configName}.json");
        if (!File.Exists(fullPath)) throw new FileNotFoundException($"Required config file not found: {fullPath}");
    ...
    private static string GetApplicationRootPath()
    {
        ...
        if (string.IsNullOrEmpty(exePath)) exePath = AppContext.BaseDirectory;
