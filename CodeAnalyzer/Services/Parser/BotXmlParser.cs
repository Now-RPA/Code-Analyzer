...
public static class BotXmlParser
{
    public static Process Parse(string xmlFilePath)
    {
        ...
        var idValue = GetElementValue(rootElement, "ID");
    ...
    private static List<SystemPlugin> GetPlugins(XmlElement parentElement)
    {
        ...
        var pluginNodes = parentElement.SelectNodes("References/AutxPluginReference")!;
    ...
    private static List<UserPlugin> GetUserPlugins(XmlElement parentElement)
    {
        ...
        var userPluginNodes = parentElement.SelectNodes("UserPlugins/AutxUserPluginReference")!;
    ...
    private static List<Activity> GetActivities(XmlElement parentElement)
    {
        ...
        var activityNodes = parentElement.SelectNodes("Activities/AutxActivity")!;
        foreach (XmlNode activityNode in activityNodes)
        {
            Activity activity = new()
            {
                ...
                MaxRetries = int.TryParse(GetElementValue(activityNode, "MaxRetries"), out var maxRetries)
                    ? maxRetries
                    : 0,
                RetryDelay = int.TryParse(GetElementValue(activityNode, "RetryDelay"), out var retryDelay)
                    ? retryDelay
                    : 0,
                OnErrorActionAfterRetry =
                    ParseEnum<OnErrorAction>(GetElementValue(activityNode, "OnErrorActionAfterRetry")),
    ...
    private static List<AbstractDesignItem> GetDesignItems(XmlNode parentNode)
    {
        ...
        var itemNodes = parentNode.SelectNodes("Items/DesignItem")!;
        foreach (XmlNode itemNode in itemNodes)
        {
            var itemType = GetAttributeValue(itemNode, "xsi:type");
            var id = ParseGuid(GetElementValue(itemNode, "ID"));
            var parentId = ParseGuid(GetElementValue(itemNode, "ParentID"));
            ...
            switch (itemType)
            {
                ...
                case "AutxCommentConnection":
                    ...
                    break;
                default:
                    var hasControlInPort = itemNode.SelectSingleNode("ControlIn") != null;
                    var hasControlOutPort = itemNode.SelectSingleNode("ControlOut") != null;

                    if (hasControlInPort || hasControlOutPort)
                    {
                        var controlIn = hasControlInPort
                            ? new ControlIn
                            {
                        ...
                        var controlOut = hasControlOutPort
                            ? new ControlOut
                            {
                        ...
                        var dataTransforms = GetDataTransforms(itemNode);
                        var mappedVariables = GetMappedVariables(itemNode);
                        ExecutableItem executableItem = new()
                        {
                            ...
                            OnErrorAction =
                                ParseEnum(GetElementValue(itemNode, "OnErrorAction"), OnErrorAction.Inherit),
                            MaxRetries = int.Parse(GetElementValue(itemNode, "MaxRetries") ?? "1"),
                            RetryDelay = int.Parse(GetElementValue(itemNode, "RetryDelay") ?? "0"),
                            OnErrorActionAfterRetry = ParseEnum(GetElementValue(itemNode, "OnErrorActionAfterRetry"),
                                OnErrorAction.Inherit),
    ...
    private static List<ActivityVariable> GetActivityVariables(XmlNode parentNode)
    {
        ...
        var variableNodes = parentNode.SelectNodes("Variables/AutxObject")!;
    ...
    private static List<GlobalVariable> GetGlobalVariables(XmlElement parentElement)
    {
        ...
        var variableNodes = parentElement.SelectNodes("Variables/AutxObject")!;
        foreach (XmlNode variableNode in variableNodes)
        {
            var variableType = GetAttributeValue(variableNode, "xsi:type") ?? string.Empty;
            var variable = variableType switch
            {
    ...
    private static List<BaseScreen> GetScreens(XmlNode parentNode)
    {
        ...
        var screenNodes = parentNode.SelectNodes("Items/AutxObject")!;
        foreach (XmlNode screenNode in screenNodes)
        {
            var screenType = GetAttributeValue(screenNode, "xsi:type") ?? string.Empty;
    ...
    private static List<BaseMatchRule> GetMatchRules(XmlNode? matchRulesNode)
    {
        ...
        {
            var ruleType = GetAttributeValue(ruleNode, "xsi:type") ?? string.Empty;
    ...
    private static List<DataTransform> GetDataTransforms(XmlNode itemNode)
    {
        ...
        var dataTransformNodes = itemNode.SelectNodes(".//DataTransform");

        if (dataTransformNodes != null)
            foreach (XmlNode dataTransformNode in dataTransformNodes)
            {
                var attributeType = GetAttributeValue(dataTransformNode, "xsi:type") ?? string.Empty;
    ...
    private static List<MappedVariable> GetMappedVariables(XmlNode itemNode)
    {
        ...
        var mappedVariableNodes = itemNode.SelectNodes(".//MappedVariable");

        if (mappedVariableNodes != null)
            foreach (XmlNode mappedVariableNode in mappedVariableNodes)
            {
                var dataIn = GetDataIn(mappedVariableNode);
                var dataOut = GetDataOut(mappedVariableNode);
    ...
    {
        var dataInNode = mappedVariableNode.SelectSingleNode("DataIn");
        if (dataInNode == null)
            return null;
    ...
    {
        var dataOutNode = mappedVariableNode.SelectSingleNode("DataOut");
        if (dataOutNode == null)
            return null;
